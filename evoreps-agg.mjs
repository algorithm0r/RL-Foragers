...
const order = ['food', 'hunt-scarce-on', 'hunt-scarce-off', 'hunt-dense-on', 'hunt-dense-off', 'shelter', 'full', 'full-pits', 'hunt-pess-on', 'hunt-opt-on'];
...
// 5) SELECTABILITY test — final attack initialQ from THREE init extremes (pess / neutral / opt). If it
// CONVERGES across inits, the gene is selectable (evolution finds an optimum); if it tracks its init, it's
// undriven drift. attack−baseline isolates attack-SPECIFIC selection (vs every action shifting together).
const labels = { 'hunt-pess-on': 'pess (~-0.9)', 'hunt-scarce-on': 'neutral (~0)', 'hunt-opt-on': 'opt (~+0.9)' };
if (byCond['hunt-pess-on'] || byCond['hunt-opt-on']) {
  console.log('\n--- instinct SELECTABILITY: final attack initialQ from opposite init points ---');
  for (const c of ['hunt-pess-on', 'hunt-scarce-on', 'hunt-opt-on']) if (byCond[c]) {
    const rs = byCond[c];
    console.log('  ' + (labels[c] || c).padEnd(14) + ' attack iQ ' + fmt(stat(rs.map((r) => r.attackInitialQ))) +
      '   attack−baseline ' + fmt(stat(rs.map((r) => r.attackInitialQ - r.meanInitialQ))) +
      '   kills ' + fmt(stat(rs.map((r) => r.greedyKills))));
  }
  console.log('  → converge across inits ⇒ SELECTABLE; track init ⇒ undriven drift');
}
