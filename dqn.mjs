// DQN vs tabular head-to-head: does a small neural net over the raw window match the layered tabular
// stack — and where does each win? Same env / reward / seeds / metric (steps-to-clear). Three settings
// probe increasing difficulty: easy base sweep, a bigger sparse arena (partial observability), and a
// 2-type sweep (the DQN input + action set both grow). → `dqn` collection.
//   node dqn.mjs [--reps N] [--ticks N] [--collection dqn] [--only substr]
...
const COLL = flag('collection', 'dqn');
const ONLY = flag('only', null);

const BASE = {
  enableShelter: false, enableWater: false, enablePits: false,
  alpha: 0.1, gamma: 0.95, confidenceK: 30, rewardStep: -1, rewardGather: 1, defaultQ: 0,
  explore: 'egreedy', epsilon: 0.01, relevanceFilter: false, strategicLayer: false,
  dqnField: 5, dqnHidden: 64, dqnAlpha: 0.0025, dqnBatch: 32, dqnReplay: 20000,
  dqnTargetSync: 1000, dqnWarmup: 1000, dqnEpsStart: 1.0, dqnEpsEnd: 0.05, dqnEpsDecaySteps: 60000,
};

// increasing difficulty: easy → bigger&sparser (partial obs) → multi-type (wider input + action set)
const SETTINGS = [
  { name: 'base-8-K1', N: 8, K: 1, nFood: 6 },
  { name: 'arena-12-K1', N: 12, K: 1, nFood: 12 },
  { name: 'types-10-K2', N: 10, K: 2, nFood: 6 },
];
let CONFIGS = [
  { name: 'dqn', agent: 'dqn' },
  { name: 'layered-135', agent: 'layered', layers: [1, 3, 5] },
  { name: 'flat-w5', agent: 'flat', receptiveField: 5 },
];
...
function runOne(set, cfg, seed, policy) {
  Object.assign(P, BASE, {
    ...
    agent: cfg ? cfg.agent : 'layered', layers: cfg && cfg.layers ? cfg.layers : [1, 3, 5],
    receptiveField: cfg && cfg.receptiveField ? cfg.receptiveField : 5,
  });
  ...
  return { final: +w.meanStepsToClear().toFixed(1), cleared: w.cleared, size: policy ? 0 : agentSize(w), nActions: w.actions.length, curve };
}
...
async function main() {
  ...
  for (const set of SETTINGS) {
    ...
    jobs.push({ set, cfg: { name: 'random' }, rep: 0, policy: () => randomInt(8 + set.K) }); // sweep: 8 moves + K collects
  }
  console.log('dqn: ' + jobs.length + ' runs, ' + TICKS + ' ticks each → ' + COLL);
  let n = 0;
  for (const j of jobs) {
    ...
    const pkt = db.packet(P, {
      experiment: 'dqn', setting: j.set.name, N: j.set.N, types: j.set.K, config: j.cfg.name, rep: j.rep, seed,
      isReference: !!j.policy, final: res.final, cleared: res.cleared, size: res.size, nActions: res.nActions, curve: res.curve,
    });
  ...
  console.log('done. ' + (await db.count(COLL, { experiment: 'dqn' })).count + ' dqn packets.');
  await db.close();
}
main().catch((e) => { console.error('dqn failed:', e); process.exit(1); });
